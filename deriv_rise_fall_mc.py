"""
+=========================================================================+
|  DERIV RISE / FALL BOT  v2  —  Monte Carlo + Jump-Diffusion            |
|                                                                         |
|  Contract type : CALL (Rise) or PUT (Fall)                             |
|  Settlement    : final tick above / below entry spot                   |
|                                                                         |
|  Signal engine (identical to RDBEAR v10):                              |
|    1. Pure GBM paths          — baseline containment / direction       |
|    2. Merton Jump-Diffusion   — fat-tail aware                         |
|    3. Blended direction probability  p_rise / p_fall                   |
|                                                                         |
|  Duration selection:                                                    |
...
CONFIG = {
    ...
    # -- EV gate ---------------------------------------------------------------
    # Pre-screen uses 0.95 as estimated payout (see _evaluate_signal).
    # This gate only rejects truly flat signals (p barely above 0.50).
    # The proposal gate (ev_check_on_proposal) makes the real decision
    # using the actual Deriv payout returned in the proposal response.
    "min_payout_ratio"     : 0.75,  # minimum real payout to accept a proposal
    "min_ev_threshold"     : 0.001, # pre-screen floor — proposal gate is stricter
    "ev_check_on_proposal" : True,  # final EV check with real payout
...
class MonteCarloPricer:
    ...
    Two models, blended:
      GBM   : dlog S = σ · Z,  Z ~ N(0,1)
      Merton: dlog S = σ_d · Z + Σ Y_k,  Y_k ~ N(μ_J, σ_J²)

    Returns (p_rise, p_fall, sigma, T) where:
        p_rise + p_fall ≈ 1.0  (small rounding difference from MC noise)
        T = number of ticks simulated
    """
    ...
    def __init__(self, cfg):
        ...
        self.p_floor        = cfg.get("mc_p_floor",         0.52)
    ...
    def simulate_direction(self, tick_buf, T: int,
        ...
        """
        Simulate T ticks forward from the current price.
        Returns (p_rise, p_fall) — probability final tick is above / below entry.
        Both models are run and blended.
        """
        if not tick_buf:
            return 0.5, 0.5
        ...
        # ── GBM ───────────────────────────────────────────────────────────
        # Final log-return of each path: sum of T iid N(0, σ²) = N(0, T·σ²)
        # (drift omitted — we have no reliable µ estimate on synthetic indices)
        gbm_final = self._rng.standard_normal(N) * sigma * math.sqrt(T)
        ...
        if ew > 0.0:
            # Diffusion-only σ (recomputed without jump ticks)
            buf     = list(tick_buf)[-(self.jd_fit_window + 1):]
            prices  = np.array([t["price"] for t in buf], dtype=float)
            ...
            # Diffusion component for T steps: N(0, T·σ_d²)
            diff_final = self._rng.standard_normal(N) * sigma_d * math.sqrt(T)
        ...
        return float(np.clip(p_rise, 0.01, 0.99)), float(np.clip(p_fall, 0.01, 0.99))
    ...
    def ev(p_win: float, payout_ratio: float) -> float:
        """Expected value per $1 staked. payout_ratio = net profit / stake."""
...
class TradeLogger:
    FIELDS = [
        ...
        "price_at_entry",
        "p_rise", "p_fall", "p_win", "ev", "payout_ratio",
        "sigma_ewma", "mc_paths",
...
class LiveTrader:
    ...
    contract with the highest EV that clears all gates.
    ...
    For each tick (after cooldowns):
      ...
      2. For each candidate duration T ∈ hold_durations:
           ...
           EV(T)       = p_win × payout(T) − (1 − p_win)
      3. Pick T* = argmax EV(T) subject to EV(T) > min_ev_threshold
                   and p_win > mc_p_floor.
    ...
    the real payout ratio before buying.  The EV gate is re-evaluated
    on the real payout; the trade is aborted if it fails.
    """
    ...
    def _evaluate_signal(self):
        cfg       = self.cfg
        p_floor   = cfg.get("mc_p_floor", 0.53)
        ev_thr    = cfg.get("min_ev_threshold", 0.02)
        durations = cfg.get("hold_durations", [1, 2, 3, 4, 5])

        # Shared σ computed once for all durations
        ...
        # Probe every candidate duration, select highest EV
        for dur in durations:
            ...
            if p_win < p_floor:
                continue   # insufficient directional edge

            # Pre-screen EV uses a neutral 0.95 estimate so duration
            # ranking is meaningful regardless of min_payout_ratio.
            # The proposal gate re-evaluates with the real Deriv payout.
            ev = MonteCarloPricer.ev(p_win, 0.95)
        ...
        if best_dur is None or best_ev < ev_thr:
            self._persist_count = max(0, self._persist_count - 1)
            if self.live_tick_count % 30 == 0:
                log.info("[MC] No edge  best_p=%.4f best_ev=%+.4f σ=%.6f "
                         "λ=%.5f  persist_decay=%d",
                         best_p, best_ev, shared_sigma,
        ...
        if self._persist_count < required:
            if self.live_tick_count % 10 == 0:
                log.info("[MC] Persistence %d/%d  %s  dur=%dt  p=%.4f  ev=%+.4f",
                         self._persist_count, required,
                         best_dir, best_dur, best_p, best_ev)
            return

        # ── All checks passed ─────────────────────────────────────────────
        jp = self.pricer.last_jump_params
        log.info("[MC] *** SIGNAL *** %s  dur=%dt  p_win=%.4f  "
                 "p_rise=%.4f  p_fall=%.4f  ev_est=%+.4f  σ=%.6f  "
                 "persist=%d | JD: λ=%.5f μ_J=%+.5f σ_J=%.5f "
                 "n_jumps=%d/%d  ew=%.2f",
                 best_dir, best_dur, best_p,
                 best_rise, best_fall, best_ev, shared_sigma,
                 self._persist_count,
        ...
        # Stake computed with estimated payout; will be re-evaluated on proposal
        stake = self.staker.next_stake(best_p, self.balance,
                                       cfg.get("min_payout_ratio", 0.80))
    ...
    def _on_proposal(self, msg):
        ...
        # Net payout ratio: profit / stake
        payout_ratio  = round((payout_amount - ask_price) / max(ask_price, 1e-8), 4)

        cfg    = self.cfg
        min_py = cfg.get("min_payout_ratio", 0.80)
        ev_thr = cfg.get("min_ev_threshold", 0.02)
        p      = self.pending_p_win

        if payout_ratio < min_py:
            log.info("[EV] SKIP — live payout=%.1f%% < min=%.0f%%",
                     payout_ratio * 100, min_py * 100)
            self.waiting_proposal = False
            return
        ...
        if cfg.get("ev_check_on_proposal", True):
            if p < cfg.get("mc_p_floor", 0.53) or ev < ev_thr:
                log.info("[EV] SKIP — p=%.4f  EV=%+.4f  (live payout=%.1f%%)",
                         p, ev, payout_ratio * 100)
                self.waiting_proposal = False
                return
        ...
        log.info("[Trader] Proposal OK  %s  dur=%dt  payout=%.1f%%  "
                 "p_win=%.4f  EV=%+.4f  stake=$%.2f",
                 self.pending_direction, self.pending_dur,
                 payout_ratio * 100, p, ev, self.pending_stake)
...
def main():
    ...
    log.info("=" * 65)
    log.info("  DERIV RISE/FALL BOT  v2  —  Monte Carlo + Jump-Diffusion")
    ...
    log.info("  p_floor     : %.3f  min_ev: %.3f  min_payout: %.0f%%",
             cfg["mc_p_floor"], cfg["min_ev_threshold"],
             cfg["min_payout_ratio"] * 100)
